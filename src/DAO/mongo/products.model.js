...
class ProductModel {
  async getAll(queryParams) {
    ...
    const products = await ProductsMongoose.paginate(filter, options);
  ...
  async getById(productId) {
    const product = await ProductsMongoose.findOne({ _id: productId }).exec();
  ...
  async getJson(queryParams) {
    ...
    const result = await ProductsMongoose.paginate(filter, options);
  ...
  async getAllProducts() {
    const products = await ProductsMongoose.find({}).exec();
  ...
  async createProduct(newProd) {
    return await ProductsMongoose.create(newProd);
  }

  async deleteProduct(productId) {
    return await ProductsMongoose.deleteOne({ _id: productId }).exec();
  }
}
